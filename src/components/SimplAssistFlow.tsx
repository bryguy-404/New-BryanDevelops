...
import styles from "./SimplAssistFlow.module.css";
...
const CONVERSATIONS: Conversation[] = [
  {
    ...
    script: [
      ...
      {
        from: "customer",
        text: "My water heater is leaking. Can someone come out this week?",
      },
      {
        from: "ai",
      ...
      {
        from: "ai",
        text: "You're booked for Tuesday at 9 AM ✅ We'll text you a reminder.",
      },
    ],
  },
  {
    ...
    script: [
      ...
      {
        from: "customer",
        text: "My check engine light came on. How much is a diagnostic?",
      },
      {
        from: "ai",
      ...
      {
        from: "ai",
        text: "You're set for 8 AM tomorrow ✅ We'll text a reminder tonight.",
      },
    ],
  },
  {
    ...
    script: [
      ...
      {
        from: "ai",
        text: "Hi! Thanks for reaching out to GreenScape 🌱 How can we help?",
      },
      {
        from: "customer",
        text: "Do you do weekly mowing? I need someone starting this month.",
      },
      {
        from: "ai",
      ...
      {
        from: "ai",
        text: "Thursday between 1–4 PM it is ✅ Our team will confirm by text in the morning.",
      },
    ],
  },
];
  ...
  return (
    <div
      className={
        from === "banner"
          ? styles.banner
          : `${styles.bubble} ${from === "customer" ? styles.customer : styles.assistant}`
      }
    >
      {children}
    </div>
...
function TypingBubble({ from }: { from: "ai" | "customer" }) {
  return (
    <div
      className={`${styles.bubble} ${styles.typing} ${from === "customer" ? styles.customer : styles.assistant}`}
      aria-hidden="true"
    >
      <span />
      <span />
...
export default function SimplAssistFlow() {
  ...
  const [animatedIndex, setActiveIndex] = useState(0);
  const [animatedVisible, setVisible] = useState(0);
  const [animatedTyping, setTyping] = useState<"ai" | "customer" | null>(null);
  const [animatedFaded, setFaded] = useState(false);
  const activeIndex = reduceMotion ? 0 : animatedIndex;
  const visible = reduceMotion
    ? CONVERSATIONS[0].script.length
    : animatedVisible;
  const typing = reduceMotion ? null : animatedTyping;
  const faded = !reduceMotion && animatedFaded;
  ...
  useEffect(() => {
    ...
    const pin = () => {
      el.scrollTop = el.scrollHeight;
      if (!reduceMotion && performance.now() - start < 650)
  ...
  useEffect(() => {
    ...
    let alive = true;
    let timer = 0;
    // Count only visible viewing time, so scrolling away or switching tabs
    // doesn't skip the conversation. Only one timeout is active at a time.
      ...
      new Promise<void>((resolve) => {
        ...
        const tick = () => {
          ...
          if (!document.hidden && inViewRef.current)
            remaining -= Math.min(now - previous, 120);
          previous = now;
  ...
  return (
    <div ref={demoRef} className={styles.frame}>
      <div
        className={styles.demo}
        data-reduced-motion={reduceMotion}
        role="group"
        aria-label="Animated example of an enquiry becoming a booking"
      >
        <div className={styles.scene}>
          ...
          <div
            className={`${styles.notice} ${styles.incoming}`}
            data-faded={faded}
          >
            <span className={styles.sourceIcon}>
              <SourceIcon size={20} strokeWidth={1.8} aria-hidden="true" />
            </span>
            <span>
              <span className={styles.cardEyebrow}>A NEW OPPORTUNITY</span>
              <strong>{outcome.source}</strong>
              <span className={styles.cardDetail}>
                {convo.lead.name} <span>· just now</span>
              </span>
            </span>
            <ArrowUpRight
              size={15}
              ...
              <div className={styles.phoneFace}>
                <div className={styles.screen}>
                  <div className={styles.statusBar} aria-hidden="true">
                    <span>{activeIndex === 2 ? "7:42" : "9:41"}</span>
                  ...
                  <div className={styles.chatHeader}>
                    ...
                    <div className={styles.businessAvatar} aria-hidden="true">
                      {outcome.initials}
                    </div>
                    <div className={styles.business} data-faded={faded}>
                      <strong>{convo.business}</strong>
                      <span>
                        <i /> Your AI receptionist
                      </span>
                    </div>
                  ...
                  <div className={styles.conversation} data-faded={faded}>
                    <p className={styles.timestamp}>
                      TODAY · {activeIndex === 2 ? "7:42 PM" : "9:41 AM"}
                    </p>
                    <div
                      ref={chatWindowRef}
                    ...
                    <div className={styles.delivery}>
                      {booked ? (
                        ...
                        <>
                          ...
                          {typing === "ai"
                            ? "SimplAssist is replying…"
                            : "SimplAssist is on it"}
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div
            className={`${styles.notice} ${styles.booking}`}
            data-booked={booked && !faded}
            aria-hidden={!booked || faded}
          >
            <div className={styles.bookingTop}>
              ...
              <span className={styles.bookingCheck}>
        ...
        <div className={styles.caption}>
          <div className={styles.journey} aria-label="Demo progress">
            <span data-active={visible > 0}>
              <SourceIcon size={13} aria-hidden="true" />
              {outcome.source}
            </span>
            <ChevronRight size={12} aria-hidden="true" />
            <span data-active={engaged}>
              <MessageCircle size={13} aria-hidden="true" />
              Replied
            </span>
            <ChevronRight size={12} aria-hidden="true" />
            <span data-active={booked} className={styles.journeyBooked}>
              <Check size={13} aria-hidden="true" />
              {activeIndex === 2 ? "Quote set" : "Booked"}
            </span>
          </div>
          {reduceMotion && (
            <span className={styles.staticLabel}>Example conversation</span>
          )}
        </div>
      </div>
    </div>
